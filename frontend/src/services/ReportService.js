...
export const reportAPI = {
  ...
  async getSubjectStatistics(subjectCode) {
    try {
      const response = await fetch(`${API_BASE_URL}/reports/statistics/subject/${subjectCode}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch subject statistics: ${response.status}`);
      }
      return await response.json();
    } catch (error) {
  ...
  async getStatisticsSummary() {
    try {
      const response = await fetch(`${API_BASE_URL}/reports/statistics/summary`);
      if (!response.ok) {
        throw new Error(`Failed to fetch statistics summary: ${response.status}`);
      }
      return await response.json();
    } catch (error) {
  ...
  async getPerformanceOverview() {
    try {
      const response = await fetch(`${API_BASE_URL}/reports/performance/overview`);
      if (!response.ok) {
  ...
  async calculateStatistics() {
    try {
      const response = await fetch(`${API_BASE_URL}/reports/statistics/calculate`, {
        method: 'POST',
        headers: {
