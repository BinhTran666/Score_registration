...
router.get(
  "/",
  GroupPerformanceController.getAvailableGroups.bind(GroupPerformanceController)
);
router.get(
  "/all/top-students",
  GroupPerformanceController.getTopPerformersAllGroups.bind(
...
router.get(
  "/:groupCode/top-students",
  GroupPerformanceController.getTopStudentsByGroup.bind(
    GroupPerformanceController
  )
);
router.get(
  "/:groupCode/statistics",
  GroupPerformanceController.getGroupStatistics.bind(GroupPerformanceController)
);
router.get(
  "/:groupCode/ranking",
  GroupPerformanceController.getGroupRanking.bind(GroupPerformanceController)
);
router.get(
  "/:groupCode/analysis",
  GroupPerformanceController.getGroupAnalysis.bind(GroupPerformanceController)
);

// Group comparison
router.post(
  "/compare",
  GroupPerformanceController.compareGroups.bind(GroupPerformanceController)
);
    ...
    for (const groupCode of groups) {
      try {
        ...
        summary[groupCode] = {
          ...
          excellence_rate:
            stats.score_distribution?.find((d) => d.label.includes("Excellent"))
              ?.percentage || 0,
        };
      } catch (error) {
        summary[groupCode] = {
